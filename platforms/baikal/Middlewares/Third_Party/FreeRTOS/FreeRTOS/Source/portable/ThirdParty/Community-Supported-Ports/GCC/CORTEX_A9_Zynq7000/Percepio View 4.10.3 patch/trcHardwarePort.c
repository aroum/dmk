...
#if ( (TRC_CFG_CORE_COUNT > 1) && (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_ARM_CORTEX_A9) )

#define CS_TYPE_ISR 2

uint32_t cortex_a9_r5_enter_critical(uint32_t* pulSavedInterruptStatus)
{
    ...
    {
    	/* The processor executes in SVC mode in an ISR other than the context-switch.
		 * Differently from a context-swtich, inside an ISR interrupts might have been enabled
		 * to allow interrupt nesting, motivating a critical section entry.
    	 * This is needed because the implementation of ulPortSetInterruptMask and vPortClearInterruptMask
		 * depends on the number of cores. In the single-core implementation vPortClearInterruptMask
		 * completely unmasks the interrupts while in multi-core it sets the ICC_PMR to the
		 * previous mask. For this reason, the former function does not need any variable to
		 * store the saved mask while the latter does. */
		*pulSavedInterruptStatus = (uint32_t) portENTER_CRITICAL_FROM_ISR();
    	cs_type = CS_TYPE_ISR;
    }
#if (TRC_CFG_RECORDER_MODE == TRC_RECORDER_MODE_STREAMING)
...
void cortex_a9_r5_exit_critical(uint32_t cs_type, uint32_t ulSavedInterruptStatus)
{
	switch (cs_type)
	{
		case CS_TYPE_TASK:
...
#else /* ((TRC_CFG_CORE_COUNT > 1) && (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_ARM_CORTEX_A9)) */
...
#endif /* ((TRC_CFG_CORE_COUNT > 1) && (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_ARM_CORTEX_A9)) */
