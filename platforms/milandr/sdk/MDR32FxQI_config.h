...
#define MDR32FxQI_SPL_VERSION                                                                                          \
    ((_MDR32FxQI_SPL_VERSION_MAJOR << 16) | (_MDR32FxQI_SPL_VERSION_MINOR << 8) | (_MDR32FxQI_SPL_VERSION_PATCH))
...
// <h> Factory data management for MCU K1986VE92xI (K1986VE92FI, K1986VE92F1I), K1986VE94GI and K1986VE1xI (K1986VE1FI,
// K1986VE1GI) <o> Factory data initialization in SystemInit() (called before entering main())
//   <0=> 0: REG_0E (Trim[2:0] bits) and REG_0F (HSI TRIM[5:0] and LSI TRIM[4:0] bits) are NOT initialized from factory
//   data. <1=> 1: REG_0E (Trim[2:0] bits) and REG_0F (HSI TRIM[5:0] and LSI TRIM[4:0] bits) are initialized from
//   factory data.
// <i> Default: 1 (Use initialization).
// <i> ATTENTION: Factory data stored in EEPROM must ALWAYS be used to ensure the correct operation of MCU. Setting this
// parameter to 0 is allowed only if the use of factory data is implemented elsewhere in the software.
...
//   <2=> 2: check enabled, source file ID, line number and checking expression (as string) are available (increased
//   code size)
