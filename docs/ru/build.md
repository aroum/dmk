...
Для запуска сборки используйте скрипт `build_all.sh` (или `build_all.ps1` в Windows через PowerShell):
...
- `--mcu [MCU]` — Выбор целевого микроконтроллера (`milandr`, `rp2040`, `rp2350`, `nrf52840`, `baikal`). По умолчанию используется контроллер, указанный в `config.h` клавиатуры (через `#define MCU <mcu>` или `#define DEFAULT_MCU <mcu>`); если он не задан, используется `milandr`. Переданный флаг `--mcu` всегда имеет наивысший приоритет и переопределяет дефолт.
...
- `--uf2` — Принудительно сгенерировать UF2-файл (примечание: `.bin` и `.uf2` генерируются автоматически при поддержке платформой).
...
2. Скопируйте собранный файл `.uf2` из папки `build/` на этот диск. Плата прошьется и перезагрузится автоматически.
...
Вы можете вынести конфигурацию своих клавиатур и кастомные модули в отдельный репозиторий (например, `my-dmk-config`) и собирать прошивки автоматически через GitHub Actions при каждом коммите без форка ядра DMK.
...
├── .github/
│   └── workflows/
│       └── build.yml       # Сценарий автоматической сборки GitHub Actions
├── keyboards/
│   ├── corne/              # Папка клавиатуры (содержит config.h)
│   │   ├── config.h
│   │   └── vial.json       # (Опционально)
│   └── magneteno/
│       ├── config.h
│       └── matrix_magneteno.c
└── modules/                # (Опционально) Пользовательские внешние модули
    └── custom_display/
        ├── module.cmake
        └── custom_display.c
```

### Пример GitHub Actions Workflow (`.github/workflows/build.yml`)

Сборка использует флаги `-DKEYBOARD_DIR` (указывает напрямую на папку клавиатуры) и `-DDMK_MODULES` (список подключаемых модулей):
