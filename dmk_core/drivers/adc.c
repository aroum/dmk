...
static inline nrf_saadc_input_t get_nrf_input(pin_t pin) {
    switch (pin) {
        ...
        default:
            if (pin < 8) return (nrf_saadc_input_t)(pin + 1);
...
void hal_adc_init(pin_t pin) {
    ...
    if (ain == NRF_SAADC_INPUT_DISABLED) return;
...
uint16_t hal_adc_read(pin_t pin) {
    ...
    if (!s_adc_inited) hal_adc_init(pin);
...
void hal_adc_init(pin_t pin) { (void)pin; }
uint16_t hal_adc_read(pin_t pin) { (void)pin; return 0; }
#endif
